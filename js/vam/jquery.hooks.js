jQuery(document).ready(function() {
    ...
    $('input#query').each(function(){
        ...
        $(this).bind('keyup', function(e){ 
            ...
            if( $(this).val().length > 1 ){
                ...
                $.ajax({ url:contentapi+'/autocomplete/keyterms/', data:$(this).serialize(), success:function(response, status){
                ...
                $.ajax({ url:contentapi+'/autocomplete/articles/', data:$(this).serialize(), success:function(response, status){
                        if( status==="success" && this.data.slice(this.data.indexOf('=')+1).replace(/\+/g,' ')===$('input#query').val() ){
    ...
    $('#content .row').each(function(){
        var w = 0, f = $(this).children('.col').last();
        $(this).children('.col').not(':last-child').each(function(){ w +=$(this).outerWidth(); });
        f.width($(this).width()-w-(f.hasClass('edge-l'))-1);
    });
    ...
    $('.lightbox.carousel').each(function(){
        ...
        cont.before($('<div class="zoom"><div class="container"/><div class="prev"/><div class="next"/></div>').width(cont.parent().width()));
    ...
    $('.figure.row .left+.figcaption').each(function(){
        $(this).css({'display':'table-cell'}).width($(this).parent().width()-$(this).parent().find('img').outerWidth());
        $(this).find('a.more').addClass('bottom-right').parent().css({'padding-bottom':'1.2em'});
    });
    
    $('.list .figure:has(.more)').each(function(){
        if( $(this).prev('.figure:has(.more)').length > 0 || $(this).next('.figure:has(.more)').length > 0 ){
    ...
    /* Landing page content area 3 more bar */
    var morebar = $("#area3-morebar");
    if ( morebar.length > 0 ) {
        var mb_html_btn = morebar.html(),
            mb_html_spinner = '<img src="http://media.vam.ac.uk/media/css/vam-css/img/spinner_666666_32x32.gif"/>',
            matrix_asset_id = $(".asset-id").attr('id');
        $.getScript("http://media.vam.ac.uk/media/js/vam-js/related_modules.js");
        var tags = '',
            count = $("a", morebar).attr('rel'),
            offset = 5,
            limit = 5,
            url_stem = '/contentapi/json/';
        search = function(tags) {
            $.ajax({
                url: url_stem + 'asset/search/quorum/all/?published=1&q=' + tags + '&limit='+limit+'&offset='+offset,
                dataType: 'json',
                headers: {'X-Asset-ID':matrix_asset_id},
                success: function(data) {
                    article_list = new Formatter('{}');
                    article_list.results = data.records;
                    html = article_list.formatListRichLinks(5);
                    $("#area3 #area3-results").append(html);
                    morebar.html(mb_html_btn);
                }
            });
        };
        $("#area3-morebar a").live('click', function(e) {
            e.preventDefault();
            $("a", morebar).html(mb_html_spinner);
            count = count - limit;
            if(count < 1) {
                count = 0;
                $("#area3-morebar a").die('click', e);
            } else {
                offset = offset + limit;
            }
            mb_html_btn = mb_html_btn.replace(/\d+/g, count);
            if( tags === ''){
                var asset_url = url_stem + 'matrixasset/'+matrix_asset_id+'/'; 
                $.getJSON(asset_url, function(data){
                    tags = data[0].extras['tags_string'];
                    search(tags);
                });
            }else{
                search(tags);
    ...
    var funnelbacklinks = $("#funnelback-links");
    if( funnelbacklinks.length > 0 ){
        $.getScript("http://media.vam.ac.uk/media/js/vam-js/related_modules.js");
        var urlstem = "http://beta.vam.ac.uk/search/json.cgi";
        fbsearch = function(query, collection){
            $.ajax({
                url:urlstem+'?query='+query+'&collection='+collection,
                dataType:'json',
                success:function(data){
                    fblink_list = new Formatter('{}');
                    fbhtml = fblink_list.formatListFunnelbackLinks(5);
                    $("#funnelback-results").html(fbhtml);
                }
            });
        };
        $("#funnelback-form").submit(function(e){
            e.preventDefault();
            var fbcollection = 'vam-topic'+$("h1").text().replace(' ', '-');
            fbsearch($('#funnelback-form input[name="query"]').val(), fbcollection);
        });
    }
